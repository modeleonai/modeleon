...
"""Tests for ``mo.Time`` — the discrete time axis.
...
and shape inference already works. Extent is start + (end | periods);
grains are day / month (the default) / quarter / year.
"""
...
class TestTimeValidation:
    ...
    def test_unknown_grain_raises(self):
        # 'week' is not a supported grain; 'fortnight' is nonsense — both refused.
        with pytest.raises(ValueError):
