...
class TestSheetNameFor:
    ...
    LayoutEngine, _build_var_to_sheet, the writer, and external consumers.
...
class TestExcelLayoutOverrides:
    """The ``excel_layout`` kwarg lets any caller override engine
    defaults via a duck-typed object — engine reads ``.sheet`` and
    ``.format`` off it without importing the caller's types. These
    tests use ``SimpleNamespace`` stubs, which is all the engine needs.
    """
    ...
    def test_layout_format_wins_over_excel_props(self, tmp_path):
        """Priority: when both are set, ``excel_layout.format`` wins
        over ``excel_props['number_format']``."""
        from types import SimpleNamespace
    ...
    def test_no_layout_falls_back_to_existing_behavior(self, tmp_path):
        ...
        assert value_cell is not None
        # excel_props applies when no layout is set.
...
class TestMultiVariableHeaderStyling:
    ...
    flatten-nested-sheets layout. Without this support, the header cell
    would land bare and a styled label would need a phantom
    ``mo.Variable(0.0, ...)`` row. With MV-level ``excel_props``
    honoured, the natural shape works.
    """
...
class TestChildrenAreSections:
    ...
    which would get one tab per record.
    ...
    @staticmethod
    def _records(mark: bool) -> mo.MultiVariable:
        records = mo.MultiVariable(display_name="Records")
        with records:
            for i in (1, 2, 3):
                rec = mo.MultiVariable(display_name=f"Rec{i}")
                with rec:
                    rec.amount = mo.Variable(100 * i)
                setattr(records, f"r{i}", rec)
        if mark:
            records._children_are_sections = True
        return records
    ...
    def test_unmarked_container_still_gets_a_tab_per_child(
        ...
        self._records(mark=False).to_excel(out)
    ...
    def test_marked_container_collapses_to_one_sheet(
        ...
        self._records(mark=True).to_excel(out)
        assert load_workbook(out).sheetnames == ["Records"]
    ...
    def test_a_child_that_claims_a_tab_still_gets_one(
        ...
        records = self._records(mark=True)
        records.r2._role = "sheet"
        out = tmp_path / "mixed.xlsx"
        records.to_excel(out)
