...
class TestBaseFontWiring:
    """The resolved view's ``base_font`` reaches the emitted cells."""
...
class TestFormatByTypeField:
    """``format_by_type`` carried by the view (legacy prop is fallback)."""
...
class TestBandStylesField:
    """Section/subsection band styling carried by the view.
...
class TestOrientTranspose:
    """``orient='down'`` transposes a flat sheet — Variables become
    columns, periods run down, formulas follow."""
    ...
    def test_default_across_unchanged(self, tmp_path):
        ...
        m.price = mo.Variable([10, 11, 12], display_name="Price")
        ws = self._ws(tmp_path, m)
        # label col A, values across B/C/D (the default layout)
        assert ws["A1"].value == "Price"
...
class TestTimelineHeader:
    ...
    def test_serializable_by_sheet_projection(self):
        ...
        # consumers read (a sheet name serializes, an id() does not); the
        # id()-keyed map stays the writer's (clone-collision-safe) source.
        # Same tuples.
...
class TestNestedSectionTitleIndent:
    def test_the_title_indents_like_its_rows_do(self, tmp_path):
        # Dense nesting shows hierarchy by indent alone (gap_rows=0), so
        # a nested section title must indent too — flush left it would
        # read as its parent's sibling.
