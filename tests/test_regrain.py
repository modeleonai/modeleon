...
"""Tests for re-grain: start=/grain=/regrain= on a Variable, and projection.

Covers the RegrainSpec constructors (mo.up / mo.frozen / mo.ratio), which
store the rule INERTLY; the time-location metadata (var.time); the
constructor guards; calendar bucketing and weighted recipes; and
``.at(grain)`` projection of single Variables and whole models.
"""
...
class TestRegrainSemantics:
    """Quotients recompute as ratio-of-sums, mo.ratio resolves siblings,
    frozen splits formula from values."""
...
class TestProjectionAt:
    ...
    def test_scalar_formula_copies_under_projection(self):
        ...
        # recompute (recompute choked on '**' and turned them into error cells).
        m = mo.Model('m')
        m.default_start = '2024-01'
        m.default_grain = 'month'
    ...
    def test_absorbed_row_keeps_source_code(self):
        # An absorbed #VALUE! cell must keep its identity — display name
        # AND the source text, so the error row still shows the code
        # that produced it.
        m = mo.Model('m')
        m.default_start = '2024-01'
        m.default_grain = 'month'
    ...
    def test_cumsum_check_row_regrains(self):
        ...
        # row. cumsum re-grains as cumsum of the summed input:
    ...
    def test_model_at_same_grain_is_identity(self):
        # A caller can ask for the NATIVE grain —
    ...
    def test_multiplicative_flow_evaluates_then_regrains(self):
        ...
        # OUTPUT is a flow, so the default evaluates at native grain and
...
class TestAliasProjection:
    ...
    follow the source instead of refusing the line."""
