...
"""Tracks — the value of a track-axised Variable.
...
A two-tier value in one type: TIME lives inside each track
(positions under the ambient window, re-grained by grain projection);
...
Keys are ROLES (machine keys) — plain strings; which names exist and
the role→label display mapping belong to the authoring layer
(``mo.Tracks``), not the value layer: this type is the generic
mechanism, the declaration lives on the model.

The broadcast law lives in ``combine``:
...
class TrackValues:
    ...
    def __init__(self, tracks: Dict[str, Fiber]) -> None:
        ...
        if len(lengths) > 1:
            raise ValueError(
                ...
                f"{sorted(lengths)}. Ragged coordinates are not "
                f"supported — pad the shorter tracks explicitly."
            )
    ...
    def combine(left: Any, right: Any,
        ...
        """Combine two operands, at least one tracked, per the broadcast law.
    ...
    def lift(fn: Callable[..., Any], *operands: Any) -> 'TrackValues':
        """Rank-lifting: map ``fn`` over roles.
