...
def reject_axised(value, where: str) -> None:
    """Rank-1 guard — refuse an axised operand where the value would
    ...
    enumerates COORDINATES. Mixing the two produces silently wrong
    numbers, so these operations refuse loudly instead of mapping per
    ...
    if axes:
        ...
        raise ValueError(
            ...
            f"coordinates with periods. Axised operands are not "
            f"supported here; slice one coordinate first."
        )
