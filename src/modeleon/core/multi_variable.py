...
Class cascade (in this file, top-to-bottom):
    ...
    MultiVariable           — universal container for direct use.
                              ...
                              without it, the MV is a plain grouping.
...
Split across two mixin modules:
...
class MultiVariableBase(_MVLifecycle, _MVInspect, Component):
    ...
    _EXCEL_PROP_KEYS = frozenset({
        'hidden',   # helper subtree — hidden from every rendering
        # Structural
        'tab',
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            excel_layout: Optional opaque layout-config object. The
                   ...
                   nothing else with it; a writer extension reads it
                   back. The engine stays duck-typed — anything that
                   round-trips through ``repr()`` is fine. ``None``
                   means inherit from parent at render time.
        ...
        # Opaque layout-config attribute. A writer extension reads this
        # back; the engine never inspects its shape. ``None`` is the
        # inherit-from-parent signal for whoever resolves it.
        ...
        # they're accessible via normal Python attribute lookup.
        # ``_component_names`` is an explicit list of which
        # ``__dict__`` keys are components —
    ...
    def _set_default_window(
        ...
        (``Variable.time``), and every renderer's timeline header is
...
class MultiVariableClass(MultiVariableBase):
    ...
    def _call_compute(self) -> None:
        ...
        # identical introspections per build for a few distinct answers.
...
class MultiVariable(MultiVariableBase):
    ...
    Pass ``excel_props={'tab': True}`` to mark an Excel tab; without it,
    the MV is a plain grouping.
    """
    ...
    def __init__(self, display_name=None, **components):
        ...
        # The tracks axis — declared once, inherited ambiently
        ...
        # DIRECTLY, so without this pop a view passed as a kwarg would
        # be adopted as CONTENT: its config leaves
        # (``totals=['quarter','year']``) would hit the extent law under
        ...
        if tracks_decl is not None:
            ...
            if not isinstance(tracks_decl, Tracks):
                raise TypeError(
                    ...
                    f"tracks=mo.Tracks('actual', 'budget'); got "
