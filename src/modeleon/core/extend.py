...
``none`` — reading beyond the values is an error; the series is not
materialized and stays partial.

A windowed model materializes a declared ``zero`` / ``hold`` extension at
ADOPTION, out to the ambient window — the same moment a schedule
materializes and the extent law runs. Arithmetic between series with
unequal extents aligns them by date; each side's cells outside its own
extent follow its declared rule, and ``none`` (or no rule) refuses.
"""
...
def none() -> ExtendRule:
    ...
    exists. The series stays partial; date-aligned arithmetic that
    reaches past its values refuses."""
