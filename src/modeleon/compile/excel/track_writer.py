...
def write_track_formulas(
    ...
    for vid, comp in by_id.items():
        ...
        if expr is None:
            ...
            if owner is None:
                continue
            if getattr(owner, "_expr_is_track_specific", False):
                # The head shows ONE authored track's expression, not
                # the line's shared formula — it says nothing about
                # this subrow's coordinate. Its numbers stand.
                continue
