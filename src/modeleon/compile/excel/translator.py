...
- :class:`~modeleon.compile.excel.renderer.ExcelRenderer` — one concrete
  renderer. Implements
...
— that callers like ``writer.py`` and ``display/html.py`` rely on,
through the public ``.translate(expr, period_idx, current_sheet,
self_address)`` API.

Other renderers (the JSON renderer in ``compile/json`` is one) sit
alongside ``ExcelRenderer`` and need no change to the walker, the AST,
or any core code.
"""
...
class ExcelTranslator:
    ...
    def translate(
        ...
        # A coordinate operand's value baked into the text (see
        # RenderCtx.inlined_value) — callers that cache formula text
        # must skip such cells.
        self.last_render_inlined = bool(ctx.inlined_value)
...
def identity_cells_for(
    ...
    address-less by id, and rendering it would unroll the whole
    expression into every consumer (e.g. a cell spelling a monthly
    subtotal out again instead of pointing at the row that already
    holds it).
    ...
    row — a plan formula referencing a blend cell would move when an
    actual lands).
    """
    ...
    for vid, var in rows:
        ...
        # ``mo.Variable([monthly expressions])`` evaluates the elements
